...
def ensure_permissions(
    ...
    Designed for use inside ``RunPython`` data migrations where Django's
    ``post_migrate`` signal may not have created default model permissions yet.
    """
...
def assign_permissions_to_group_in_migration(
    apps: Any,
    group_name: str,
    ...
    Designed for use inside ``RunPython`` data migrations.
    """
...
def remove_permissions_from_group_in_migration(
    apps: Any,
    group_name: str,
    permission_map: dict[type[Model], list],
) -> None:
    ...
    Reverse of ``assign_permissions_to_group_in_migration``.
    """
    Group = apps.get_model("auth", "Group")
    Permission = apps.get_model("auth", "Permission")
    ContentType = apps.get_model("contenttypes", "ContentType")
