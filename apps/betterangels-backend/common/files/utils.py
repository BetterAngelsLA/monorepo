...
def infer_attachment_type(mime_type: Optional[str]) -> AttachmentType:
    """
    Map MIME type → AttachmentType.

    Falls back to DOCUMENT if unknown or missing.
    """
    if not mime_type:
        return AttachmentType.DOCUMENT

    prefix = mime_type.split("/", 1)[0].lower()
    return _MIME_PREFIX_TO_TYPE.get(prefix, AttachmentType.DOCUMENT)
