...
class PermissionedQuerySetImplicitOrgTestCase(TestCase):
    """``allow_implicit_org=True`` stands in for a header the client never sent.

    It adopts the caller's only organization, and refuses when there is not
    exactly one — no organization to adopt, or no way to choose between two.
    """

    def setUp(self) -> None:
        self.org_1 = organization_recipe.make(name="pqs_implicit_org_1")
        self.org_2 = organization_recipe.make(name="pqs_implicit_org_2")
    ...
    def _extension(self) -> PermissionedQuerySet:
        return PermissionedQuerySet(
            model=Note,
            perms=[NotePermissions.CHANGE],
            organization_field="organization_id",
            allow_implicit_org=True,
        )

    def _prepare(self, organization_id: Any, user: Optional[User] = None) -> Any:
        return self._extension()._prepare_qs(stub_info(user or self.user, organization_id))

    def test_headerless_adopts_the_callers_only_organization(self) -> None:
        self.assertEqual(list(self._prepare(None)), [self.note_1])

    def test_the_adopted_organization_is_visible_to_the_resolver_body(self) -> None:
        """The body re-reads ``request.organization_id``; adoption has to reach it."""
        info = stub_info(self.user, None)

        self._extension()._prepare_qs(info)

        self.assertEqual(info.context.request.organization_id, str(self.org_1.pk))
    ...
    def test_headerless_denies_a_caller_in_no_organization(self) -> None:
        orphan = user_with_global_perm(NotePermissions.CHANGE)

        with self.assertRaises(DjangoNoPermission):
            self._prepare(None, user=orphan)

    def test_the_header_still_decides_when_it_is_sent(self) -> None:
        self.org_2.add_user(self.user)

        self.assertEqual(list(self._prepare(self.org_2.pk)), [self.note_2])

    def test_a_malformed_header_still_denies(self) -> None:
        """Only an absent header is adopted; an unusable one is a client error."""
        with self.assertRaises(DjangoNoPermission):
            list(self._prepare("not-an-id"))
