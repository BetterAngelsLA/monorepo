...
class UpdateReferralOrgScopingTestCase(GraphQLBaseTestCase):
    ...
    def test_without_the_org_header_adopts_the_callers_organization(self) -> None:
        ...
        response = self._update("Updated headerless")

        self.assertEqual(response["data"]["updateReferral"]["notes"], "Updated headerless")

    def test_without_the_org_header_still_denies_another_organizations_referral(self) -> None:
        other_org_referral = baker.make(Referral, organization=self.org_2)
        self.graphql_client.defaults.pop("HTTP_X_ORGANIZATION_ID", None)

        response = self.execute_graphql(
            UPDATE_REFERRAL, {"data": {"id": str(other_org_referral.pk), "notes": "Should not update"}}
        )
