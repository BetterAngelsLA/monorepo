...
if TYPE_CHECKING:
    ...
    from shelters.types.reporting import DailyBedStatusMetricsType, DailyOccupancyMetricsType, ReservationMetricsType
...
def bed_status_metric(
    *,
    ...
    """Return daily bed status counts for each day in the inclusive date range.
    ...
    bed as of end-of-day in the caller's timezone.  When a bed's status changes
    ...
    *start* and *end* are timezone-aware datetimes.  The caller's timezone
    ...
    # Replay all events before the range to build the initial snapshot.
    split = next((i for i, e in enumerate(tagged) if e["pgh_created_at"] >= start), len(tagged))
    ...
    for day in days:
        ...
        while event_idx < total_in_range and in_range_events[event_idx]["pgh_created_at"] < day_end:
