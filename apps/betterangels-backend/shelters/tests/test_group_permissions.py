...
class ShelterGroupPermissionsTestCase(TestCase):
    ...
    def _group_codenames(self, model: str) -> set[str]:
        return set(
            self.permission_group.group.permissions.filter(
    ...
    def test_sync_populates_an_empty_group_and_is_idempotent(self) -> None:
        self.permission_group.group.permissions.clear()

        sync_group_permissions()
        first_pass = set(self.permission_group.group.permissions.values_list("codename", flat=True))
        self.assertIn("view_shelter", first_pass)
        self.assertIn("change_shelter", first_pass)

        sync_group_permissions()
        self.assertSetEqual(set(self.permission_group.group.permissions.values_list("codename", flat=True)), first_pass)

    def test_member_gains_shelters_admin_access(self) -> None:
        user = baker.make(User, is_staff=True)
        self.permission_group.group.user_set.add(user)
        self.permission_group.group.permissions.clear()
