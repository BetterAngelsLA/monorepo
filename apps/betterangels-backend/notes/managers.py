...
class MoodQuerySet(QuerySet):
    def delete(self) -> Tuple[int, Dict[str, int]]:
        """
        Overriding this method so that when the moods from a single
        Note instance are deleted, the associated Note can have it's
        updated_at timestamp reflect the change in moods, in order
        to be able to roll back via simple history.
        """
