...
import { ErrorHmis, InvalidFileTypeErrorHmis } from './errorHmis';
import {
  ALLOWED_FILE_TYPES,
...
class ClientHmis {
  ...
  private async getBaseUrl(): Promise<string> {
    const apiUrlHmis = await AsyncStorage.getItem(HMIS_API_URL_STORAGE_KEY);
    if (!apiUrlHmis) {
      throw new ErrorHmis(
      ...
      case 422: {
        ...
        if (validationData?.messages) {
          const errors = Object.entries(validationData.messages)
            ...
            .join(', ');
          throw new ErrorHmis(`Validation errors: ${errors}`, 422, data);
        }
        throw new ErrorHmis('Validation error', 422, data);
      }
      default:
        throw new ErrorHmis(
  ...
  get<T = unknown>(path: string, params?: HttpQueryParamsHmis): Promise<T> {
 ...
 *
 * @throws InvalidFileTypeErrorHmis
 ...
 * @example
 * ```ts
 * const result = await clientHmis.uploadClientFile({
 *   clientId: '68998C256',
 *   file: {
 ...
 */
  async uploadClientFile(
    ...
    if (!ALLOWED_FILE_TYPES.includes(file.mimeType)) {
      throw new InvalidFileTypeErrorHmis('Invalid file type', 400, {
   ...
   * @throws ErrorHmis if the request fails
   *
   * @example
   * ```typescript
   * // Update only metadata
   * const result = await clientHmis.updateClientFile(
   *   '68998C256',
   *   42,
   *   12, // new category ID
   *   89  // new file name ID
   ...
   * const resultWithFile = await clientHmis.updateClientFile(
   *   '68998C256',
   *   42,
   *   12,
   *   89,
   ...
   * @example
   * ```typescript
   * const response = await clientHmis.getFileCategories();
   ...
   * @example
   * ```typescript
   * const response = await clientHmis.getFileNames({
   *   page: 1,
   *   per_page: 50,
   *   sort: 'name'
   * });
   ...
   * ```
   */
  async getFileNames(params?: HttpQueryParamsHmis): Promise<FileNamesResponse> {
   ...
   * @example
   * ```typescript
   * const files = await clientHmis.getClientFiles('68998C256', {
   *   sort: '-file_date',
   *   page: 1,
   *   per_page: 10,
   ...
   * @example
   * ```typescript
   * await clientHmis.deleteClientFile('68998C256', 37);
   * ```
   */
   ...
   * await clientHmis.uploadClientPhoto('68998C256', formData);
   * ```
   */
...
// Factory function to create ClientHmis
export const createClientHmis = () => new ClientHmis();
