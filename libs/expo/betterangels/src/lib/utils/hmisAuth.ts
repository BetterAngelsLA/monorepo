...
const storage = createMMKV({ id: 'hmis-auth' });

const HMIS_DOMAIN_KEY = 'hmis_domain';
const HMIS_REFRESH_URL_KEY = 'hmis_refresh_url';
...
export const getHmisDomain = (): string | null => {
  return storage.getString(HMIS_DOMAIN_KEY) ?? null;
};
...
export const getHmisRefreshUrl = (): string | null => {
  return storage.getString(HMIS_REFRESH_URL_KEY) ?? null;
};
