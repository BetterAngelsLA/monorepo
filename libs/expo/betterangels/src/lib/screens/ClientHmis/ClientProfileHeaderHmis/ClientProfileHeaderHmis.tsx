...
export function ClientProfileHeaderHmis(props: IClientHeaderProps) {
  ...
  const { thumbnailUri, headers } = useClientPhotoContentUriHmis(clientId);

  const nameParts = [firstName, nameMiddle, lastName].filter((s) => !!s);

  const visibleSuffix = getExistingSuffixHmis(nameSuffix as HmisSuffixEnum);
