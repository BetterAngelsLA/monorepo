import { ErrorHmis, ReactNativeFile } from '@monorepo/expo/shared/clients';
...
import { useClientHmis } from '../../hooks';
...
export default function RestDebugHmis() {
  const {
    getCurrentUser,
    uploadClientFile,
    getFileCategories,
  ...
  const handlePhotoUpload = useCallback(
    async (file: ReactNativeFile) => {
      ...
      try {
        ...
        const result = await clientHmis.postMultipart(endpoint, formData);
  ...
  const handlePhotoCrop = useCallback(async () => {
    ...
    try {
      ...
      const result = await clientHmis.post(endpoint, payload);
  ...
  }, [cropClientId, cropX1, cropY1, cropW, cropH, clientHmis]);
