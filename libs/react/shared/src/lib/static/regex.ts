export const Regex = {
  ...
  /** US phone with loose formatting: optional +1/1 prefix, spaces/dashes/dots/parens, a valid 10-digit NANP number, and optional x/ext/extension. */
  phoneNumberLooseUS:
    /^\+?1?[\s.-]*(?:\([2-9]\d{2}\)|[2-9]\d{2})[\s.-]*[2-9]\d{2}[\s.-]*\d{4}(?:\s*(?:x|ext|extension)\.?\s*\d+)?$/i,
