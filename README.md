...
- `graphql`: This folder contain our GraphQL queries and mutations. `gql-types` will contain the output TypeScript types based off these mutations and queries.
- `rest-graphql`: This folder containes `apollo-rest-link` queries and mutations. These queries & mutation do not generate TypeScript types, though in the future we can either manually create our own GraphQL schema.
- `gql-types`: This folder contains autogenerated GraphQL types. These types are generated from our GraphQL schema and are crucial for type safety in our frontend application. They should always be kept up to date with the latest schema.
