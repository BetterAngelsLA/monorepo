...
// 2. Find existing builds for this fingerprint
function findBuildId(platform: string): string {
  console.log(
    `\nLooking for ${platform} build with runtime version: ${fingerprint}`
  );
  try {
    const builds = runJson<Array<{ id: string }>>(
    ...
    if (builds.length > 0) {
      console.log(`Found ${platform} build: ${builds[0].id}`);
      return builds[0].id;
    }
  } catch {
    // no builds found
  ...
  throw new Error(
    `No ${platform} build found for fingerprint ${fingerprint}. ` +
      'Run the deploy pipeline first.'
  );
}

const androidBuildId = findBuildId('android');
const iosBuildId = findBuildId('ios');
